define([
    ...
    'streamhub-structs/content/types/twitter-search-struct-content',
    'streamhub-sdk/content/content',
    'streamhub-sdk/content/types/livefyre-content'
], function (jasmine, StructContent, TwitterSearchStructContent, Content, LivefyreContent) {
    describe('streamhub-structs/content/types/twitter-search-struct-content', function () {
        describe('when constructed from a twitter search struct state', function () {
            var content,
                twitterStructState;
            beforeEach(function () {
                ...
                content = new TwitterSearchStructContent(twitterStructState);
            });
            it('is instanceof TwitterSearchStructContent', function () {
                ...
                expect(content instanceof TwitterSearchStructContent).toBe(true);
            });
            it('has .id', function () {
                expect(content.id).toBe(twitterStructState.content.id);
            });
            it('has .tweetId', function () {
                expect(content.tweetId).toBe('357958778538303488');
            });
            it('has .body', function () {
                var twitterProcessedContent = "Check out skye8844's comment in \"Hitting Rock Bottom with Joe and Teresa\" <a href=\"http://t.co/3kMcXjubqa\" title=\"http://fyre.it/1NQ9\" rel=\"nofollow\"><span class='tco-ellipsis'><span style='position:absolute;left:-9999px;'>&nbsp;</span></span><span style='position:absolute;left:-9999px;'>http://</span><span class='js-display-url'>fyre.it/1NQ9</span><span style='position:absolute;left:-9999px;'></span><span class='tco-ellipsis'><span style='position:absolute;left:-9999px;'>&nbsp;</span></span></a>";
                expect(content.body).toBe(twitterProcessedContent);
            });
            it('has .author info', function () {
                expect(content.author.id).toBe("1323349722@twitter.com");
                expect(content.author.displayName).toBe("Rhonj Season 5");
                expect(content.author.avatar).toBe("http://a0.twimg.com/profile_images/378800000007901456/227c7637b5f4cc1ec1fc055021e5d4ba_normal.jpeg");
                expect(content.author.profileUrl).toBe("https://twitter.com/rhonj_5");
                expect(content.author.screenName).toBe('rhonj_5');
                expect(content.author.twitterUserId).toBe(1323349722);
            });
        });

        it('can be constructed with no options', function () {
            expect(new TwitterSearchStructContent() instanceof TwitterSearchStructContent).toBe(true);
        });
        ...
        describe('when constructed with a non-struct state', function () {
            ...
            it('throws StructContent.InvalidStructState', function () {
                expect(function () {
                    content = new TwitterSearchStructContent(nonStructState);
        ...
        describe('when constructed with a struct state of wrong type', function () {
            ...
            it('throws', function () {
                expect(function () {
                    new TwitterSearchStructContent(plaintextStructState);
                }).toThrow();
            });
        });
    });
